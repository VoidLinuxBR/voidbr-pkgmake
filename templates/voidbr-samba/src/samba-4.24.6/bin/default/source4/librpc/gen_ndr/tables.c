
/* Automatically generated by tables.pl. DO NOT EDIT */

#include "includes.h"
#include "librpc/ndr/libndr.h"
#include "librpc/ndr/ndr_table.h"
#include "librpc/gen_ndr/ndr_drsuapi.h"
#include "librpc/gen_ndr/ndr_epmapper.h"
#include "librpc/gen_ndr/ndr_eventlog.h"
#include "librpc/gen_ndr/ndr_browser.h"
#include "librpc/gen_ndr/ndr_dssetup.h"
#include "librpc/gen_ndr/ndr_frsapi.h"
#include "librpc/gen_ndr/ndr_spoolss.h"
#include "librpc/gen_ndr/ndr_dnsserver.h"
#include "librpc/gen_ndr/ndr_samr.h"
#include "librpc/gen_ndr/ndr_srvsvc.h"
#include "librpc/gen_ndr/ndr_winreg.h"
#include "librpc/gen_ndr/ndr_mgmt.h"
#include "librpc/gen_ndr/ndr_netlogon.h"
#include "librpc/gen_ndr/ndr_svcctl.h"
#include "librpc/gen_ndr/ndr_wkssvc.h"
#include "librpc/gen_ndr/ndr_eventlog6.h"
#include "librpc/gen_ndr/ndr_backupkey.h"
#include "librpc/gen_ndr/ndr_witness.h"
#include "librpc/gen_ndr/ndr_clusapi.h"
#include "librpc/gen_ndr/ndr_mdssvc.h"
#include "librpc/gen_ndr/ndr_echo.h"
#include "librpc/gen_ndr/ndr_fsrvp.h"
#include "librpc/gen_ndr/ndr_lsa.h"
#include "librpc/gen_ndr/ndr_winspool.h"
#include "librpc/gen_ndr/ndr_atsvc.h"
#include "librpc/gen_ndr/ndr_gkdi.h"
#include "librpc/gen_ndr/ndr_initshutdown.h"
#include "librpc/gen_ndr/ndr_ntsvcs.h"
#include "librpc/gen_ndr/ndr_dfs.h"
#include "librpc/gen_ndr/ndr_unixinfo.h"
#include "librpc/gen_ndr/ndr_audiosrv.h"
#include "librpc/gen_ndr/ndr_dbgidl.h"
#include "librpc/gen_ndr/ndr_dsbackup.h"
#include "librpc/gen_ndr/ndr_efs.h"
#include "librpc/gen_ndr/ndr_frstrans.h"
#include "librpc/gen_ndr/ndr_frsrpc.h"
#include "librpc/gen_ndr/ndr_keysvc.h"
#include "librpc/gen_ndr/ndr_msgsvc.h"
#include "librpc/gen_ndr/ndr_orpc.h"
#include "librpc/gen_ndr/ndr_policyagent.h"
#include "librpc/gen_ndr/ndr_rot.h"
#include "librpc/gen_ndr/ndr_scerpc.h"
#include "librpc/gen_ndr/ndr_trkwks.h"
#include "librpc/gen_ndr/ndr_w32time.h"
#include "librpc/gen_ndr/ndr_winstation.h"
#include "librpc/gen_ndr/ndr_wzcsvc.h"
#include "librpc/gen_ndr/ndr_bkupblobs.h"
#include "librpc/gen_ndr/ndr_cab.h"
#include "librpc/gen_ndr/ndr_file_id.h"
#include "librpc/gen_ndr/ndr_fscc.h"
#include "librpc/gen_ndr/ndr_fsrvp_state.h"
#include "librpc/gen_ndr/ndr_named_pipe_auth.h"
#include "librpc/gen_ndr/ndr_negoex.h"
#include "librpc/gen_ndr/ndr_notify.h"
#include "librpc/gen_ndr/ndr_ntprinting.h"
#include "librpc/gen_ndr/ndr_ODJ.h"
#include "librpc/gen_ndr/ndr_printcap.h"
#include "librpc/gen_ndr/ndr_rap.h"
#include "librpc/gen_ndr/ndr_smb2_lease_struct.h"
#include "librpc/gen_ndr/ndr_ioctl.h"
#include "librpc/gen_ndr/ndr_nfs4acl.h"
#include "librpc/gen_ndr/ndr_quota.h"
#include "librpc/gen_ndr/ndr_wsp.h"
#include "librpc/gen_ndr/ndr_auth.h"
#include "librpc/gen_ndr/ndr_claims.h"
#include "librpc/gen_ndr/ndr_dcerpc.h"
#include "librpc/gen_ndr/ndr_dfsblobs.h"
#include "librpc/gen_ndr/ndr_dns.h"
#include "librpc/gen_ndr/ndr_dnsp.h"
#include "librpc/gen_ndr/ndr_drsblobs.h"
#include "librpc/gen_ndr/ndr_gmsa.h"
#include "librpc/gen_ndr/ndr_idmap.h"
#include "librpc/gen_ndr/ndr_keycredlink.h"
#include "librpc/gen_ndr/ndr_krb5pac.h"
#include "librpc/gen_ndr/ndr_krb5ccache.h"
#include "librpc/gen_ndr/ndr_schannel.h"
#include "librpc/gen_ndr/ndr_messaging.h"
#include "librpc/gen_ndr/ndr_misc.h"
#include "librpc/gen_ndr/ndr_nbt.h"
#include "librpc/gen_ndr/ndr_ntlmssp.h"
#include "librpc/gen_ndr/ndr_preg.h"
#include "librpc/gen_ndr/ndr_bcrypt_rsakey_blob.h"
#include "librpc/gen_ndr/ndr_security.h"
#include "librpc/gen_ndr/ndr_server_id.h"
#include "librpc/gen_ndr/ndr_smb_acl.h"
#include "librpc/gen_ndr/ndr_tpm20_rsakey_blob.h"
#include "librpc/gen_ndr/ndr_xattr.h"
#include "librpc/gen_ndr/ndr_smb3posix.h"
#include "librpc/gen_ndr/ndr_winbind.h"
#include "source4/librpc/gen_ndr/ndr_ntp_signd.h"
#include "source4/librpc/gen_ndr/ndr_opendb.h"
#include "source4/librpc/gen_ndr/ndr_sasl_helpers.h"
#include "source4/librpc/gen_ndr/ndr_winsif.h"
#include "source4/librpc/gen_ndr/ndr_winsrepl.h"
#include "source4/librpc/gen_ndr/ndr_irpc.h"
#include "source3/librpc/gen_ndr/ndr_open_files.h"
#include "source3/librpc/gen_ndr/ndr_perfcount.h"
#include "source3/librpc/gen_ndr/ndr_secrets.h"
#include "source3/librpc/gen_ndr/ndr_smbXsrv.h"
#include "source3/librpc/gen_ndr/ndr_leases_db.h"
#include "source3/librpc/gen_ndr/ndr_rpcd_witness.h"

NTSTATUS ndr_table_register_builtin_tables(void)
{
	NTSTATUS status;

	status = ndr_table_register(&ndr_table_drsuapi);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_epmapper);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_eventlog);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_browser);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dssetup);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_frsapi);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_spoolss);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dnsserver);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_samr);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_srvsvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_winreg);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_mgmt);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_netlogon);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_svcctl);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_wkssvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_eventlog6);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_backupkey);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_witness);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_clusapi);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_mdssvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_rpcecho);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_FileServerVssAgent);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_lsarpc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_iremotewinspool);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_atsvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_gkdi);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_initshutdown);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ntsvcs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_netdfs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_unixinfo);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_audiosrv);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dbgidl);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ad_backup);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ad_restore);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_efs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_frstrans);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_frsrpc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_keysvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_msgsvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_msgsvcsend);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ObjectRpcBaseTypes);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_policyagent);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_rot);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_scerpc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_trkwks);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_w32time);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_winstation);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_wzcsvc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_bkupblobs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_cab);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_file_id);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_fscc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_fsrvp_state);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_named_pipe_auth);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_negoex);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_notify);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ntprinting);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ODJ);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_printcap);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_rap);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smb2_lease_struct);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_copychunk);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_compression);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_netinterface);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_sparse);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_resiliency);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_trim);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_fsctl);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_nfs4acl_interface);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_file_quota);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smb2_query_quota);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smb1_nt_transact_query_quota);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_msftewds);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_auth);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_claims);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dcerpc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dfsblobs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dns);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_dnsp);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_drsblobs);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_gmsa);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_idmap);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_keycredlink);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_krb5pac);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_krb5ccache);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_schannel);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_messaging);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_misc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_nbt);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ntlmssp);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_preg);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_bcrypt_rsakey_blob);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_security);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_server_id);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smb_acl);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_tpm20_rsakey_blob);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_xattr);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smb3posix);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_winbind);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_ntp_signd);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_opendb);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_sasl_helpers);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_winsif);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_winsrepl);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_irpc);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_open_files);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_perfcount);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_secrets);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_smbXsrv);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_leases_db);
	if (NT_STATUS_IS_ERR(status)) return status;

	status = ndr_table_register(&ndr_table_rpcd_witness);
	if (NT_STATUS_IS_ERR(status)) return status;


	
	return NT_STATUS_OK;
}
